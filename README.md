...
[Electricity Cost Predictor](https://electricity-cost-predictor-5d88feda62f8.herokuapp.com/) is a machine learning (ML) project that uses a publicly available dataset to determine whether a regression pipeline can be built to estimate monthly electricity cost for different site profiles.
This was achieved by training a supervised regression model using `electricity_cost` as the target variable and site and operational characteristics (e.g., site area, utilisation rate, water consumption, and resident count) as features.
...
- **Hypothesis 2 - Operational intensity is a strong cost driver**
   ...
   - **Statistical Evidence:** Boxplot comparisons showed that sites without residents tend to have a slightly higher **median electricity cost**, likely reflecting higher baseline energy usage in commercial or industrial settings. However, sites with residents exhibit a wider cost distribution and several high-cost outliers, indicating that residential occupancy can substantially increase electricity consumption under certain conditions. Additionally, both `resident_count` and `utilisation_rate` showed positive correlation values with electrictiy cost, supporting the hypothesis that **operational intensity - rather than site type alone - is a key drvier of electricity expenditure**..
